const searchFieldGetDebt = document.querySelector('#searchFieldGetDebt');
const tableOutputGetDebt = document.querySelector('.tableOutputGetDebt');
const appTableGetDebt = document.querySelector('.appTableGetDebt');

tableOutputGetDebt.style.display = "none";


searchFieldGetDebt.addEventListener("keyup", (e) => {
	const searchValue = e.target.value;

	if(searchValue.trim().length > 0) {
		console.log("searchValue", searchValue);
		tableOutputGetDebt.innerHTML = "";
		fetch("/search-product", {
			...
			.then((data) => {
				...
				if(data.length == 0){
					appTableGetDebt.style.display = "none";
					tableOutputGetDebt.style.display = "block";


					tableOutputGetDebt.innerHTML = "Mahsulot topilmadi!";
		...
		tableOutputGetDebt.style.display = "none";
		appTableGetDebt.style.display = "block";
	}
});